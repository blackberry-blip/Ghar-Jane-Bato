  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          <h2 style={{ color: '#0D1B3E', margin: '0 0 4px 0' }}>📋 सबै बुकिङहरू</h2>
          <p style={{ color: '#666', margin: 0, fontSize: '14px' }}>
            {pendingCount > 0 ? <span style={{ color: '#C0182A', fontWeight: 'bold' }}>⚠️ {pendingCount} वटा पर्खाइमा छन्</span> : 'सबै बुकिङहरू हेरिएका छन्'}
          </p>
        </div>
        <button onClick={() => setBookings(JSON.parse(localStorage.getItem('bookings') || '[]').reverse())} style={{ padding: '8px 16px', background: '#0D1B3E', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px' }}>🔄 रिफ्रेस</button>
      </div>
