...
function SearchableDropdown({ label, placeholder, options, value, onChange, icon }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', marginBottom: '16px' }}>
      <label style={{ display: 'block', marginBottom: '6px', fontWeight: '600', color: '#0D1B3E', fontSize: '14px' }}>{label}</label>
      <div onClick={() => setOpen(true)} style={{ display: 'flex', alignItems: 'center', padding: '14px', borderRadius: '12px', border: '2px solid #e0dcd3', background: '#fff', cursor: 'pointer', fontSize: '16px', color: value ? '#0D1B3E' : '#999' }}>
        <span style={{ marginRight: '10px', fontSize: '20px' }}>{icon}</span>
        <span>{value || placeholder}</span>
      </div>
      {open && (
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '6px', background: '#fff', borderRadius: '12px', border: '1px solid #e0dcd3', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', zIndex: 100, maxHeight: '280px', overflow: 'auto' }}>
          <input autoFocus type="text" value={filter} onChange={(e) => setFilter(e.target.value)} placeholder="खोज्नुहोस्..." style={{ width: '100%', padding: '12px 16px', border: 'none', borderBottom: '1px solid #eee', fontSize: '15px', outline: 'none', boxSizing: 'border-box' }} />
          {filtered.map((city) => (
            <div key={city} onClick={() => { onChange(city); setOpen(false); setFilter('') }} style={{ padding: '12px 16px', cursor: 'pointer', borderBottom: '1px solid #f5f5f5', fontSize: '15px', color: '#0D1B3E' }} onMouseEnter={(e) => e.target.style.background = '#fff8e1'} onMouseLeave={(e) => e.target.style.background = '#fff'}>{city}</div>
          ))}
          {filtered.length === 0 && <div style={{ padding: '16px', color: '#999', textAlign: 'center' }}>केही भेटिएन</div>}
        </div>
      )}
    </div>
  )
...
function BusCard({ bus, onSelect }) {
  return (
    <div style={{ background: '#fff', borderRadius: '16px', padding: '16px', marginBottom: '12px', border: '1px solid #e8e4db', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
        ...
        <span style={{ background: bus.available < 10 ? '#ffebee' : '#e8f5e9', color: bus.available < 10 ? '#C0182A' : '#2e7d32', padding: '4px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold' }}>{bus.available} सीट बाँकी</span>
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={() => onSelect(bus)} style={{ padding: '10px 24px', background: '#0D1B3E', color: '#fff', border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '600', cursor: 'pointer' }}>छान्नुहोस्</button>
      </div>
    </div>
  )
}
...
function BookingModal({ bus, from, to, onClose, onConfirm }) {
  ...
  const handleProceed = () => {
    if (!form.name || !form.phone || !form.date) { alert('कृपया सबै विवरण भर्नुहोस्'); return }
    setShowPay(true)
  }
  ...
  if (showPay) {
    return (
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '16px', zIndex: 200 }}>
        <div style={{ background: '#fff', borderRadius: '20px', padding: '28px', width: '100%', maxWidth: '400px', textAlign: 'center' }}>
          <div style={{ fontSize: '56px', marginBottom: '16px' }}>🙏</div>
          <h2 style={{ color: '#0D1B3E', margin: '0 0 8px 0' }}>बुकिङ पक्का गर्नुहोस्</h2>
          <p style={{ color: '#666', margin: '0 0 20px 0', fontSize: '14px' }}>{form.name} जी, तपाईंको बुकिङ सुरक्षित छ</p>
          <div style={{ background: '#f5f5f5', borderRadius: '12px', padding: '16px', marginBottom: '20px', textAlign: 'left' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}><span style={{ color: '#666' }}>मार्ग</span><span style={{ fontWeight: '600' }}>{from} → {to}</span></div>
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'flex-end', zIndex: 200 }}>
      <div style={{ background: '#fff', borderRadius: '24px 24px 0 0', padding: '24px', width: '100%', maxWidth: '480px', maxHeight: '90vh', overflow: 'auto' }}>
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <input required type="text" value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} placeholder="जस्तै: राम बहादुर" style={{ width: '100%', padding: '14px', borderRadius: '10px', border: '1px solid #ddd', fontSize: '16px', boxSizing: 'border-box' }} />
        </div>
        <div style={{ marginBottom: '16px' }}>
          <label style={{ display: 'block', marginBottom: '6px', fontWeight: '600', color: '#333', fontSize: '14px' }}>फोन नम्बर *</label>
          <input required type="tel" value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} placeholder="+91 98765 43210" style={{ width: '100%', padding: '14px', borderRadius: '10px', border: '1px solid #ddd', fontSize: '16px', boxSizing: 'border-box' }} />
        </div>
        <div style={{ display: 'flex', gap: '12px', marginBottom: '16px' }}>
          <div style={{ flex: 1 }}>
            <label style={{ display: 'block', marginBottom: '6px', fontWeight: '600', color: '#333', fontSize: '14px' }}>सीट *</label>
            <select value={form.seats} onChange={(e) => setForm({...form, seats: e.target.value})} style={{ width: '100%', padding: '14px', borderRadius: '10px', border: '1px solid #ddd', fontSize: '16px', boxSizing: 'border-box', background: '#fff' }}>
              {[1,2,3,4,5].map(n => <option key={n} value={n}>{n}</option>)}
            </select>
          </div>
          <div style={{ flex: 2 }}>
            <label style={{ display: 'block', marginBottom: '6px', fontWeight: '600', color: '#333', fontSize: '14px' }}>यात्रा मिति *</label>
            <input required type="date" value={form.date} onChange={(e) => setForm({...form, date: e.target.value})} style={{ width: '100%', padding: '14px', borderRadius: '10px', border: '1px solid #ddd', fontSize: '16px', boxSizing: 'border-box' }} />
          </div>
        </div>
        <div style={{ background: '#f5f5f5', borderRadius: '12px', padding: '16px', marginBottom: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 'bold', fontSize: '18px' }}><span>कुल रकम</span><span style={{ color: '#C0182A' }}>₹{total}</span></div>
        </div>
        <button onClick={handleProceed} style={{ width: '100%', padding: '16px', background: '#0D1B3E', color: '#fff', border: 'none', borderRadius: '12px', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer' }}>अगाडि बढ्नुहोस् →</button>
      </div>
    </div>
  )
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch('/api/buses').then(r => r.json()).then(d => {
      setAllBuses(d || [])
      setLoading(false)
    })
  }, [])
  ...
  const handleSearch = () => {
    if (!fromCity || !toCity) { alert('कृपया दुवै शहर छान्नुहोस्'); return }
      ...
      {!showWelcome && step === 'search' && (
        <div style={{ maxWidth: '480px', margin: '0 auto', padding: '24px 16px' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: '20px', marginBottom: '40px' }}>
            <a href="/admin" style={{ color: '#aaa', fontSize: '12px', textDecoration: 'none' }}>व्यवस्थापक प्यानल →</a>
          </div>
        </div>
      )}
      {!showWelcome && step === 'results' && (
        <div style={{ maxWidth: '480px', margin: '0 auto', padding: '16px' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <h2 style={{ color: '#0D1B3E', margin: '0 0 4px 0', fontSize: '20px' }}>{fromCity} → {toCity}</h2>
            <p style={{ color: '#666', margin: 0, fontSize: '14px' }}>{filteredBuses.length} वटा बस भेटियो</p>
          </div>
          {filteredBuses.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px', background: '#fff', borderRadius: '16px' }}>
              ...
              <button onClick={() => setStep('search')} style={{ marginTop: '16px', padding: '10px 20px', background: '#0D1B3E', color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>अर्को मार्ग खोज्नुहोस्</button>
            </div>
          ) : filteredBuses.map(bus => <BusCard key={bus.id} bus={bus} onSelect={handleSelectBus} />)}
